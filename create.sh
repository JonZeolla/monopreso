...
# Escape sed-replacement metacharacters (backslash, ampersand, the % delimiter)
# so titles like "Foo & Bar" don't get expanded into the matched pattern.
sed_escape() { printf '%s' "$1" | sed -e 's/[\\&%]/\\&/g'; }
TITLE_ESC="$(sed_escape "${TITLE}")"
FOLDER_ESC="$(sed_escape "${FOLDER}")"
sed -i '' -e "s%modules/TODO%modules/${FOLDER_ESC}%g" -e "s%<h1>TODO</h1>%<h1>${TITLE_ESC}</h1>%g" "${FOLDER_DIR}/${FOLDER}_content.j2"
